...
# import src.process_script.process_script as input
...
def get_all_counts(df_script):
    """
    Aggregate lines and words by season, episode and across all series

    Args:
        df_script (pd.DataFrame): dataset containing one line per row
    Returns:
        pd.DataFrame: aggregated dataframe with all types of aggregation
                        possible
    """
    logger.info(f"Getting all word and line counts")
